...
async function proximoDiaHabil(desde = new Date(), buscarDiaNoHabil = (fecha) => prisma.diaNoHabil.findUnique({ where: { fecha } })) {
  ...
  for (let intento = 0; intento < 30; intento++) {
    ...
    if (!esFinDeSemana) {
      const marcado = await buscarDiaNoHabil(fecha);
